...
def sel_cat(rad,cat_stars,cat_splus,filt,photoflag):
	...
	mag_obj = []
	for i in idx_1:
		mag_star.append(starcat['B2mag'][i])
	for j in idx_2:
...
def plot_radial_bins2(r,cat_stars,cat_splus,filt,photoflag,magstar_min,magstar_max):
	...
	return globals()["data" + str(1)],globals()["data" + str(2)],globals()["data" + str(3)],globals()["data" + str(4)],globals()["data" + str(5)]

def main():

	catstars = os.path.join(home,'data/iDR2/STRIPE82_USNO_stars_broad.fits')
	catfile = os.path.join(home,'data/iDR2/STRIPE82_magtest_broad.fits')
	filt = 'r'
	photoflag = 0

	magstar_min = np.arange(14.0,15.0,1)
	...
	for i in magstar_min:
		...
		for j in range(5):
			...
			for r in radius:
				...
				df0,df1,df2,df3,df4 = plot_radial_bins2(r,catstars,catfile,filt,photoflag,i,i+1)
				df0_in,df1_in,df2_in,df3_in,df4_in = plot_radial_bins2(r-2,catstars,catfile,filt,photoflag,i,i+1)
				print(len(df4))
