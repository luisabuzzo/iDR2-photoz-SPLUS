...
def check_star_in_border(cat_stars,photoflag,survey,distance_to_border):
	...
	splus_tiles_file = os.path.join(home,'data/all_pointings.csv')
	...
	cat_stars_near_stars = starcat[stars_near_stars]

	# for some reason the stars_near_borders indices are floats and appear in an ndarray, not a list
	stars_near_borders_list = list(np.int_(stars_near_borders).flatten())
	cat_stars_near_border = starcat[stars_near_borders_list]

	return coords_stars, starcat, cat_stars_near_stars, cat_stars_near_border
...
def main():
	...
	if getpass.getuser() == "luisabuzzo" or getpass.getuser() == "mlgbuzzo":	
	...
	if getpass.getuser() == "roderik":	
		catstars = os.path.join(home,'data/match_STRIPE82_GSC.fits')
	...
	starcat, starcat_orig, stars_near_stars, stars_near_borders = check_star_in_border(catstars,photoflag,survey,distance_to_border)

	# show the samples of stars on the sky (can be rather slow, and you need to zoom in to see it well!)
	make_skyplot_of_stars = 0
	if make_skyplot_of_stars == 1:
		# plot the selected stars on the footprint map
		dust_map = 'data/lambda_sfd_ebv.fits'
		bgmap = H.read_map(home + dust_map)
		make_skyplot(moll=True, ecliptic=True, footprint=True, idr2=False, title='S-PLUS', highlight=False, highlight_survey='', highlight_tile='', path='./', bgmap=bgmap)
		plot_objects(starcat_orig['RA'],starcat_orig['Dec'],symbol='p',color='k',markersize=3) # plot them on the map
		plot_objects(starcat['RA'],starcat['Dec'],symbol='2',color='b',markersize=8) # plot them on the map
		plot_objects(stars_near_borders['RA'],stars_near_borders['Dec'],symbol='x',color='r',markersize=10) # plot them on the map
		plot_objects(stars_near_stars['RA'],stars_near_stars['Dec'],symbol='+',color='g',markersize=10) # plot them on the map
		#plt.savefig('starcounts_stars.png',transparent=True,dpi=600)
		plt.show()
